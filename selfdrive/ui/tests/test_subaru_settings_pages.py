...
def test_mici_subaru_layout_contains_driving_only_subaru_controls():
  ...
  assert 'BigParamControl("full manual\\nyield release", "MCSubaruManualYieldFullReleaseEnabled")' not in source
...
def test_subaru_params_and_metadata_match_brand_scoped_defaults():
  ...
  assert '"MCSubaruManualYieldFullReleaseEnabled"' not in metadata_source
  assert '"title": "Full Manual Yield Release"' not in metadata_source
  assert 'Release Subaru LKAS/EPS while manual yield is active' not in metadata_source
